...
def get_computer_info():
    ...
    info["win"] = str(os.environ['SystemRoot'])  # Windows directory path
    ...
    info["current_drive"] = str(pathlib.Path.home().drive)  # Label of the current drive
    return info

def calculate_hash(computer_info):
    """Calculate SHA256 hash of computer info."""
...
def write_to_registry(signature):
    """Write signature to registry."""
    try:
        last_name = "Sliepyi2"  
        key_path = fr"Software\{last_name}"
...
def pad_data(data):
    """Pad data to be AES block size compliant."""
    padder = padding.PKCS7(algorithms.AES.block_size).padder()
    padded_data = padder.update(data) + padder.finalize()
    return padded_data

def unpad_data(padded_data):
    """Unpad AES padded data."""
...
class UserManagementApp:
    ...
    def login(self):
        ...
        if not username:
            ...
            return
        user_data = load_user_data(encrypted=True)
    ...
    def change_admin_password(self):
        ...
        if current_password != user_data["users"]["ADMIN"]["password"]:
        ...
        if new_password != confirm_password:
            ...
            return
        user_data["users"]["ADMIN"]["password"] = new_password
    ...
    def view_users(self):
        user_data = load_user_data(encrypted=True)
        users = user_data["users"]
        users_list = "\n".join([f"{user}: {details}" for user, details in users.items()])
    ...
    def change_password(self):
        ...
        if current_password != user_data["users"][self.current_user]["password"]:
            messagebox.showerror("Error", "Incorrect current password!")
            return
        ...
        if user_data["users"][self.current_user]["password_restrictions"]:
            if not re.search(r'[,.:;!?]', new_password):
                ...
                return
            if not re.search(r'[1234567890]', new_password):
            ...
            if not re.search(r'[\*\(\)\-\+\%=/]', new_password):
                ...
                return
        user_data["users"][self.current_user]["password"] = new_password
        save_user_data(user_data, encrypted=True)
    ...
    def show_about(self):
        messagebox.showinfo("About", "Author: Roman Sliepyi\nVersion: 1.0\nTask: Implement a user management system. Restrictions: Наявність цифр, розділового і знаків арифметичних операцій знаків")
